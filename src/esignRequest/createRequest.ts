...
const createSignedXMLCallback = async (data: ISignedParam, callback: Function) => {
    const { pfxFile, password, xml , rootElementName } = data
    pem.readPkcs12(pfxFile, { p12Password: password }, (err, data) => {
        ...
        sig.addReference(`//*[local-name(.)='${rootElementName}']`, ["http://www.w3.org/2000/09/xmldsig#enveloped-signature"], "http://www.w3.org/2000/09/xmldsig#sha1", "", "", "", true)
        //@ts-ignore
        sig.keyInfoProvider = {
